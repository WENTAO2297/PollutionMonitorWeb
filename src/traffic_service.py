...
def get_amap_api_key():
    """Read the AMap key without requiring local secrets to exist."""
    api_key = os.getenv("AMAP_API_KEY") or os.getenv("AMAP_KEY")
    if api_key:
        return api_key

    try:
        return st.secrets.get("AMAP_API_KEY") or st.secrets.get("AMAP_KEY")
    except Exception:
        return None
...
def get_landmarks_traffic(landmarks_list):
    ...
    api_failed = not amap_key # 标记高德 API 是否已阵亡

    if not amap_key:
        print("未配置高德 API Key，使用离线模拟交通数据。")
